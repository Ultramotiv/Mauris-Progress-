...
<head>
    ...
    <style>
        ...
        #debug { position: absolute; top: 140px; left: 20px; color: #0f0; font-size: 14px; background: rgba(0,0,0,0.7); padding: 10px; border-radius: 5px; font-family: monospace; }
        ...
        #coin-popup {
            position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
            color: #ffd700; font-size: 64px; font-weight: bold; text-align: center;
...
<body>
    <div id="info">
        <div>HIGHWAY COIN COLLECTOR</div>
        <div style="font-size: 18px;">Angle: <span id="joint6">0.0</span> degrees | Speed: 10 km/h</div>
        <div style="font-size: 20px; color: #ffd700;">
            Collected: <span id="coins">0</span> | 
            Missed: <span id="passed-coins">0</span> | 
            Accuracy: <span id="accuracy">0%</span>
        </div>
        <div style="font-size: 18px; color: #aaa;">Score: <span id="score">0</span>pts</div>
    </div>
    <div id="debug">
        <div>Connecting...</div>
        <div>Force: <span id="force-mz">0.0</span> N·m</div>
        <div>Velocity: <span id="velocity">0.0</span> deg/s</div>
        <div style="color: yellow;">-50 degrees to RIGHT | 140 degrees to LEFT</div>
    </div>
    <div id="dashboard">
        <div class="gauge">
    ...
    <div id="gameover">
        <div>GAME OVER</div>
        <div style="font-size: 24px; margin-top: 20px;">Final Score: <span id="final-score">0</span>pts</div>
        <div style="font-size: 20px; margin-top: 10px; color: #ffd700;">
            Collected: <span id="final-coins">0</span> | Missed: <span id="final-passed">0</span>
        </div>
        <div style="font-size: 18px; margin-top: 8px; color: #0f0;">Accuracy: <span id="final-accuracy">0%</span></div>
    ...
    <script>
        ...
        const playerBody = new THREE.Mesh(new THREE.BoxGeometry(1.9, 0.6, 4.2), new THREE.MeshLambertMaterial({ color: 0xdd0000 }));
        playerBody.position.y = 0.4; playerBody.castShadow = true; playerCar.add(playerBody);
        const playerCabin = new THREE.Mesh(new THREE.BoxGeometry(1.7, 0.5, 2.0), new THREE.MeshLambertMaterial({ color: 0xbb0000 }));
        playerCabin.position.set(0, 0.85, -0.3); playerCabin.castShadow = true; playerCar.add(playerCabin);
        const playerHood = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.15, 1.5), new THREE.MeshLambertMaterial({ color: 0xff1111 }));
        playerHood.position.set(0, 0.625, -1.35); playerCar.add(playerHood);
        const windshield = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.4, 0.8), new THREE.MeshLambertMaterial({ color: 0x111144, transparent: true, opacity: 0.5 }));
        windshield.position.set(0, 0.95, -0.7); windshield.rotation.x = -0.2; playerCar.add(windshield);
        const rearWindow = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.35, 0.6), new THREE.MeshLambertMaterial({ color: 0x111144, transparent: true, opacity: 0.5 }));
        rearWindow.position.set(0, 0.9, 0.5); rearWindow.rotation.x = 0.15; playerCar.add(rearWindow);
        [-0.6, 0.6].forEach(x => {
            const light = new THREE.Mesh(new THREE.CircleGeometry(0.15, 16), new THREE.MeshBasicMaterial({ color: 0xffffaa }));
            light.position.set(x, 0.5, -2.05); light.rotation.y = Math.PI; playerCar.add(light);
        });
        [-0.6, 0.6].forEach(x => {
            const light = new THREE.Mesh(new THREE.CircleGeometry(0.12, 16), new THREE.MeshBasicMaterial({ color: 0xff0000 }));
            light.position.set(x, 0.45, 2.05); playerCar.add(light);
        });
        const spoiler = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.1, 0.4), new THREE.MeshLambertMaterial({ color: 0x111111 }));
        ...
        playerPositions.forEach(pos => {
            ...
            const rim = new THREE.Mesh(new THREE.CircleGeometry(0.2, 16), new THREE.MeshLambertMaterial({ color: 0x888888 }));
        ...
        function createAICar() {
            ...
            const aiBody = new THREE.Mesh(new THREE.BoxGeometry(aiCarWidth, 0.6, aiCarLength), new THREE.MeshLambertMaterial({ color: config.bodyColor }));
            aiBody.position.y = 0.4; aiBody.castShadow = true; aiCar.add(aiBody);
            const aiCabin = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.5, 2.2), new THREE.MeshLambertMaterial({ color: config.cabinColor }));
            aiCabin.position.y = 0.85; aiCabin.castShadow = true; aiCar.add(aiCabin);
            const frontWindow = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.35, 0.7), new THREE.MeshLambertMaterial({ color: 0x222244, transparent: true, opacity: 0.4 }));

# code created on 1 Nov, 
# 0-100 robot based 30/45/60% 
#DONE
import sys
...
class RobotControlGUI:
    def __init__(self, root):
        ...
        # ------------------------------------------------------------------
        # Robot / FT / CSV / Trial / Patient variables
        # ------------------------------------------------------------------
        self.robot = None
        ...
        self.csv_start_joints = None
        self.csv_end_joints = None
        self.csv_trajectory = []
        ...
        # ------------------------------------------------------------------
        # Layout – scrollable container (keeps everything visible)
        # ------------------------------------------------------------------
        ...
        # Title
        # ------------------------------------------------------------------
        title = ttk.Label(scrollable_frame, text="ROBOT CONTROL PANEL",
                         font=('Arial', 18, 'bold'))
        title.grid(row=0, column=0, columnspan=3, pady=(0, 20), sticky='ew')
        scrollable_frame.columnconfigure(0, weight=1)

        # ------------------------------------------------------------------
        # CSV File Selection
        # ------------------------------------------------------------------
        csv_frame = ttk.LabelFrame(scrollable_frame, text=" CSV Trajectory File ", padding=12)
        csv_frame.grid(row=1, column=0, sticky='ew', pady=8, padx=5)

        self.csv_label = ttk.Label(csv_frame, text="No file selected", foreground="red")
        self.csv_label.grid(row=0, column=0, padx=8, sticky='w')
        self.btn_browse = ttk.Button(csv_frame, text="Browse CSV", command=self.browse_csv_file)
        self.btn_browse.grid(row=0, column=1, padx=8)

        # ------------------------------------------------------------------
        # Main Control Buttons
        # ------------------------------------------------------------------
        btn_frame = ttk.Frame(scrollable_frame)
        btn_frame.grid(row=2, column=0, pady=12, sticky='ew')
        btn_frame.columnconfigure((0,1), weight=1)

        self.btn_elbow_flexion = ttk.Button(btn_frame, text="Elbow Flexion", command=self.elbow_flexion_action)
        self.btn_elbow_flexion.grid(row=0, column=0, padx=10, pady=5, sticky='ew')
        self.btn3 = ttk.Button(btn_frame, text="Button 3", command=self.button3_action)
        self.btn3.grid(row=0, column=1, padx=10, pady=5, sticky='ew')

        # ------------------------------------------------------------------
        # Trajectory Movement
        # ------------------------------------------------------------------
        traj_frame = ttk.LabelFrame(scrollable_frame, text=" Move to Trajectory Position ", padding=12)
        traj_frame.grid(row=3, column=0, sticky='ew', pady=8, padx=5)

        ttk.Label(traj_frame, text="Degrees (0-100):").grid(row=0, column=0, padx=8, pady=5, sticky='w')
        self.degree_entry = ttk.Entry(traj_frame, width=12, font=('Arial', 11))
        self.degree_entry.grid(row=0, column=1, padx=8, pady=5)
        self.degree_entry.insert(0, "30")
        self.btn_move_trajectory = ttk.Button(traj_frame, text="Move to Position",
                                             command=self.move_to_trajectory_position)
        self.btn_move_trajectory.grid(row=0, column=2, padx=8, pady=5)
        self.btn_move_trajectory.config(state='disabled')

        info = ttk.Label(traj_frame, text="(CSV Start=0°, End=100°)",
                         font=('Arial', 9, 'italic'), foreground="gray")
        info.grid(row=1, column=0, columnspan=3, pady=2)

        # ------------------------------------------------------------------
        # Force/Torque Recording
        # ------------------------------------------------------------------
        ft_frame = ttk.LabelFrame(scrollable_frame, text=" Force/Torque Recording ", padding=12)
        ft_frame.grid(row=4, column=0, sticky='ew', pady=8, padx=5)
        ...
        patient_frame.grid(row=5, column=0, sticky='ew', pady=12, padx=5)
        ...
        result_frame.grid(row=6, column=0, sticky='ew', pady=12, padx=5)
        ...
        # Startup log
        # ------------------------------------------------------------------
        self.log_message("System initialized (default theme).")

    # ----------------------------------------------------------------------
    # Helper: console-only logging
    # ----------------------------------------------------------------------
    ...
    def browse_csv_file(self):
        ...
        if path:
            ...
            self.csv_label.config(text=f"Selected: {os.path.basename(path)}", foreground="green")
            self.log_message(f"CSV selected: {path}")
            self.load_csv_trajectory()
    ...
    def load_csv_trajectory(self):
        try:
            with open(self.csv_file_path, 'r') as f:
                ...
                self.csv_trajectory = [[float(r[i]) for i in range(1,7)]
                                      for r in reader if len(r) >= 7]
            if self.csv_trajectory:
                self.csv_start_joints = self.csv_trajectory[0].copy()
                self.csv_end_joints   = self.csv_trajectory[-1].copy()
                self.log_message(f"Trajectory loaded – {len(self.csv_trajectory)} points")
        except Exception as e:
            self.log_message(f"CSV load error: {e}")

    # ----------------------------------------------------------------------
    # Button enable / disable helpers
    # ----------------------------------------------------------------------
    def disable_buttons(self):
        for w in (self.btn_elbow_flexion, self.btn3, self.btn_move_trajectory,
                  self.btn_record, self.btn_start_trial, self.btn_save):
            w.config(state='disabled')

    def enable_buttons(self):
        self.btn_elbow_flexion.config(state='normal')
        self.btn3.config(state='normal')
        if self.robot and self.csv_start_joints:
    ...
    def start_next_trial(self):
        if not self.monitoring_active:
            messagebox.showwarning("Warning", "Run 'Elbow Flexion' first.")
            return
    ...
    def move_to_trajectory_position(self):
        if not self.robot:
            ...
            return
        try:
            deg = float(self.degree_entry.get())
            if not 0 <= deg <= 100: raise ValueError
        except:
            messagebox.showerror("Error", "Enter a number 0-100")
            return
        self.disable_buttons()
        threading.Thread(target=self.run_trajectory_move, args=(deg,), daemon=True).start()

    def run_trajectory_move(self, deg):
        try:
            f = deg / 100.0
            target = [self.csv_start_joints[i] +
                     (self.csv_end_joints[i] - self.csv_start_joints[i]) * f
                     for i in range(6)]
            ret = self.robot.MoveJ(joint_pos=target, vel=4.28, tool=0, user=0,
                                   desc_pos=[0]*7, acc=0, ovl=100,
                                   exaxis_pos=[0]*4, blendT=-1,
                                   offset_flag=0, offset_pos=[0]*6)
            time.sleep(0.5)
            if ret == 0:
                _, pos = self.robot.GetActualJointPosDegree()
                self.log_message(f"Moved to {deg}°")
        except Exception as e:
    ...
    # ----------------------------------------------------------------------
    # FT sensor initialisation / calibration / monitoring
    # ----------------------------------------------------------------------
    ...
    def start_ft_monitoring(self):
        ...
        self.btn_start_trial.config(state='normal', text="Start Trials")
    ...
    def elbow_flexion_action(self):
        if not self.csv_file_path:
            ...
            return
        self.disable_buttons()
        threading.Thread(target=self.run_elbow_flexion, daemon=True).start()
    ...
    def run_elbow_flexion(self):
        try:
            ...
            self.init_ft_sensor()
            target = self.csv_start_joints.copy()
            ret = self.robot.MoveJ(joint_pos=target, vel=4.28, tool=0, user=0,
                                   desc_pos=[0]*7, acc=0, ovl=100,
                                   exaxis_pos=[0]*4, blendT=-1,
                                   offset_flag=0, offset_pos=[0]*6)
            ...
            if ret == 0:
                ...
                self.log_message("Ready – start trials.")
        except Exception as e:
    ...
    def button3_action(self):
        self.disable_buttons()
        threading.Thread(target=lambda: (time.sleep(1), self.enable_buttons()), daemon=True).start()

...
class ExerciseTrialPage(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        # Mode Selection (fixed to Isometric only)
        mode_frame = tk.Frame(container, bg='#f5f6fa')
        mode_frame.pack(fill='x', pady=(0, 10))

        tk.Label(mode_frame, text="Mode:", font=('Inter', 16, 'bold'), bg='#f5f6fa').pack(side='left', padx=(0, 20))
        # Only Isometric mode is available for exercises
        tk.Label(mode_frame, text="Fixed Isometric Muscle Testing", font=('Inter', 16), bg='#f5f6fa').pack(side='left')
    ...
    def plot_trial_graph(self, trial_num):
        ...
        if self.mode.get() == "isometric":
            ...
            # Early RFD window: 50-100 ms after onset (use stored onset index if available)
            onset_idx = data.get("sidx") if data.get("sidx") is not None else 0
            ...
            # Plot Early RFD (50-100 ms) on the same graph instead of force at 3s
            if idx_100 > idx_50 and len(forces) > 1:
                early_rfd = data.get("early_rfd", 0.0)
                early_rfd_kg = early_rfd / 9.81 if early_rfd else 0.0
            ...
            # Early RFD (50-100 ms)
            early_val = data.get("early_rfd", 0.0)
            add_row("Early RFD (50-100ms)", f"{early_val:.1f}", "N/s", "#7518a3")
        ...
        for t in self.trial_results:
            ...
            if self.mode.get() == "isometric":
                row.update({
                    ...
                    "Early_RFD_N/s": f"{r.get('early_rfd', 0):.0f}",
                })
            else:
                row.update({
...
class ProgressReportCard(tk.Frame):
    # Minimal required columns; Average/Peak may have alternate names (handled during parsing)
    REQUIRED = {"Timestamp", "Name", "Exercise"}
    ...
    def _plot(self):
        ...
        try:
            with open("results.csv", "r", encoding="utf-8") as f:
                ...
                # require only the minimal base columns; other numeric fields are optional
                fieldset = set(reader.fieldnames)
                missing_base = self.REQUIRED - fieldset
                if missing_base:
                    raise ValueError(f"Missing columns: {', '.join(missing_base)}")
                for r in reader:
                    if r["Name"].strip().lower() == name.lower():
                        # robust parsing of fields saved by app
                        ts_full = r.get("Timestamp", "")
                        trial_date = ts_full.split()[0] if ts_full else ""
                        def fval(*keys, default=0.0):
                            for k in keys:
                                if k in r and r[k] != "":
                                    try:
                                        return float(r[k])
                                        ...
                                        try:
                                            return float(r[k].split()[0])
                                        except:
                                            pass
                            return default

                        avg = fval("Average", "Average_N", default=0.0)
                        peak = fval("Peak", "Peak_N", default=0.0)
                        start_force = fval("Start_Force_N", "Start_Force", default=0.0)
                        time_to_peak = fval("Time_to_Peak_s", "Time_to_Peak", default=0.0)
                        early_rfd = fval("Early_RFD_N/s", "Early_RFD", default=0.0)
    ...
    def _draw_all_exercises(self, rows):
        ...
        for ex, ex_rows in ex_data.items():
            ...
            # Keep figure for PDF export, but do not embed bar graph in the report page
            self.figures.append(fig)

            # ---- Per-trial detailed report cards for this exercise ---------
            # Sort trials for this exercise by timestamp (newest first if available)
            trials = sorted(ex_rows, key=lambda x: x.get('timestamp') or x.get('date'), reverse=True)

            # Container for reports for this exercise
            rep_container = tk.Frame(self.graph_frame, bg='white', relief='solid', bd=1)
            rep_container.pack(fill='x', pady=(0, 15), padx=10)
            tk.Label(rep_container, text=f"{ex} — Detailed Trials", font=('Inter', 12, 'bold'), bg='white').pack(anchor='w', padx=12, pady=(8, 6))

            for tr in trials:
                card = tk.Frame(rep_container, bg='#fbfbfb', relief='groove', bd=1)
                card.pack(fill='x', padx=12, pady=8)
                header = tk.Frame(card, bg='#fbfbfb')
                header.pack(fill='x', pady=(8, 0), padx=8)
                ts = tr.get('timestamp') or tr.get('date')
                trial_label = tr.get('trial') or ''
                tk.Label(header, text=f"Timestamp: {ts}  {('• Trial ' + str(trial_label)) if trial_label else ''}",
                         font=('Inter', 10, 'italic'), bg='#fbfbfb').pack(side='left')

                # rows: Peak, Average, Start Force, Time to Peak, Early RFD
                def small_row(parent, label, val, unit):
                    r = tk.Frame(parent, bg='#fbfbfb')
                    r.pack(fill='x', padx=8, pady=2)
                    tk.Label(r, text=label, font=('Inter', 10), bg='#fbfbfb').pack(side='left')
                    tk.Label(r, text=f"{val} {unit}", font=('Inter', 10, 'bold'), bg='#fbfbfb').pack(side='right')

                peak_N = tr.get('peak', 0.0)
                avg_N = tr.get('avg', 0.0)
                start_N = tr.get('start_force', 0.0)
                ttp = tr.get('time_to_peak', 0.0)
                early = tr.get('early_rfd', 0.0)

                # display numeric rows
                small_row(card, 'Peak Force', f"{peak_N:.2f}", 'N')
                small_row(card, 'Average Force', f"{avg_N:.2f}", 'N')
                small_row(card, 'Start Force (0 s)', f"{start_N:.2f}", 'N')
                small_row(card, 'Time to Peak', f"{ttp:.2f}", 's')
                small_row(card, 'Early RFD (50-100ms)', f"{early:.1f}", 'N/s')

                # Progress bar for peak force: 0-25 kg (convert N -> kg)
                peak_kg = peak_N / 9.81 if peak_N else 0.0
                peak_kg_clamped = min(25.0, peak_kg)
                pb_frame = tk.Frame(card, bg='#fbfbfb')
                pb_frame.pack(fill='x', padx=8, pady=(6, 10))
                tk.Label(pb_frame, text='Peak (kg):', font=('Inter', 10), bg='#fbfbfb').pack(side='left')
                pb = ttk.Progressbar(pb_frame, orient='horizontal', length=400, mode='determinate', maximum=25.0)
                pb['value'] = peak_kg_clamped
                pb.pack(side='left', padx=8)
                tk.Label(pb_frame, text=f"{peak_kg:.2f} kg ({peak_N:.1f} N)", font=('Inter', 10), bg='#fbfbfb').pack(side='left', padx=8)

...
###Need tunnign Constant Resistance code 
...
running = True
fixed_tcp_ref = None
original_joint_pos = None  # Store home position for spring-back
...
ORIGINAL_K = [10.0, 10.0, 10.0, 10.0, 10.0, 10.0]  # Spring stiffness per joint [J1, J2, J3, J4, J5, J6]
IK_TO_SERVO_RATIO = 2
...
MAX_JOINT_VELOCITY = 30.0  # deg/s (safe operating speed)
...
def control_loop():
    ...
    global filtered_desired_joints, fixed_tcp_ref, original_joint_pos
    ...
    print(" M-B-K spring system tries to return robot to home position")
    ...
    if j[0] == 0:
        ...
        original_joint_pos = j[1][:6]  # Store home position
    ...
    try:
        while running:
            ...
            if ik_count >= IK_TO_SERVO_RATIO:
                ...
                # Apply admittance control with M-B-K spring system
                # This creates a virtual spring that pulls robot back to home position
                for j in range(6):
                    # Position error from IK target
                    ik_err = avg_joints[j] - desired_joint_pos[j]
                    
                    # Spring force pulling back to HOME position (creates spring-back effect)
                    spring_force = ORIGINAL_K[j] * (original_joint_pos[j] - desired_joint_pos[j])
                    
                    # IK tracking force (follows the admittance motion)
                    ik_force = ik_err * 3.9
                    
                    # Total force = IK tracking + Spring restoring force
                    total_force = ik_force + spring_force
                    
                    # Mass-Spring-Damper dynamics: F = Ma + Bv
                    acc = (total_force - B[j] * joint_velocity[j]) / M[j]
                ...
                if servo_count % 20 == 0:
                    ...
                    # Calculate max spring displacement from home
                    max_spring_disp = max(abs(desired_joint_pos[i] - original_joint_pos[i]) for i in range(6))
                    print(f"Fy={filtered_fy_world:+6.2f}N → Y={current_tcp[1]:8.2f}mm | "
                          f"Resistance={resistance:+6.2f}mm | SpringDisp={max_spring_disp:5.2f}° | "
...
if __name__ == "__main__":
    ...
    init_ft_sensor()
    calibrate_baseline()
    print("\nSTARTING PURE Y-AXIS ADMITTANCE WITH M-B-K SPRING-BACK")
    print("Push/pull the tool along Y → M-B-K system pulls back to home position!")
    print("Release the tool → robot returns to starting position like a spring!")
    try:
        control_loop()

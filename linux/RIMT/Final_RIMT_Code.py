...
<body>
    ...
    <div class="exercise-page" id="exercisePage">
        ...
        <div class="exercise-content">
            ...
            <div class="exercise-two-col">
                ...
                <div class="side-main">
                    <div style="width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); border-radius: 6px; padding: 12px; margin-bottom: 15px; box-sizing: border-box;">
                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; font-size: 12px;">
                        <div><strong style="color:#2ecc71;">Fz:</strong> <span id="liveFzLeft">--</span> N</div>
                        <div><strong>J4:</strong> <span id="j4Left">--</span>°</div>
                        <div><strong>J5:</strong> <span id="j5Left">--</span>°</div>
                        <div><strong>J6:</strong> <span id="j6Left">--</span>°</div>
                    </div>
                    </div>
                    </div>
                ...
                <div class="side-main">
                    <div style="width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); border-radius: 6px; padding: 12px; margin-bottom: 15px; box-sizing: border-box;">
                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; font-size: 12px;">
                        <div><strong style="color:#2ecc71;">Fz:</strong> <span id="liveFzRight">--</span> N</div>
                        <div><strong>J4:</strong> <span id="j4Right">--</span>°</div>
                        <div><strong>J5:</strong> <span id="j5Right">--</span>°</div>
                        <div><strong>J6:</strong> <span id="j6Right">--</span>°</div>
                    </div>
                    </div>
    ...
    <div class="exercise-page" id="progressReportPage">
        <div class="exercise-header">
            ...
            <div style="display:flex; gap:10px; align-items:center;">
                ...
                <button class="btn btn-secondary" onclick="printProgressReport()">🖨️ Print Report</button>
            </div>
        </div>
    ...
    <script>
        ...
        function printProgressReport() {
            ...
            const content = document.getElementById('progressReportData').innerHTML;
            ...
            printWindow.document.write(`
                ...
                <head>
                    <title>${title}</title>
                    <style>
                        body { font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; margin: 20px; background: #0c2d29; color: white; }
                        .report-container { max-width: 100%; }
                        .header { text-align: center; margin-bottom: 20px; padding: 20px; border-bottom: 2px solid rgba(255,255,255,0.2); }
                        .exercise-title { font-size: 24px; font-weight: bold; margin-bottom: 15px; color: white; }
                        .side-title { font-size: 18px; font-weight: bold; margin-bottom: 10px; color: white; }
                        .trial-card { margin: 10px 0; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 5px; border: 1px solid rgba(255,255,255,0.1); }
                        .force-bar { height: 20px; background: rgba(255,255,255,0.1); margin: 5px 0; position: relative; border-radius: 4px; }
                        .force-value { position: absolute; right: 5px; top: 0; bottom: 0; display: flex; align-items: center; font-weight: bold; color: white; }
                        .metrics-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 10px; }
                        .metric-box { background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); border-radius: 4px; padding: 10px; }
                        .metric-label { font-size: 11px; color: rgba(255,255,255,0.7); }
        ...
        // Live data polling for Fz and joint positions
        ...
        try {
            ...
            // Update both sides with same live robot/sensor data
            // (Fz and joints are system-wide, not side-specific)
            document.getElementById('liveFzLeft').textContent = (data.fz || 0).toFixed(2);
            document.getElementById('liveFzRight').textContent = (data.fz || 0).toFixed(2);

            document.getElementById('j4Left').textContent = (data.joints?.[3] || 0).toFixed(1); // J4 = index 3
            document.getElementById('j5Left').textContent = (data.joints?.[4] || 0).toFixed(1); // J5 = index 4
            document.getElementById('j6Left').textContent = (data.joints?.[5] || 0).toFixed(1); // J6 = index 5

            document.getElementById('j4Right').textContent = (data.joints?.[3] || 0).toFixed(1);
            document.getElementById('j5Right').textContent = (data.joints?.[4] || 0).toFixed(1);
            document.getElementById('j6Right').textContent = (data.joints?.[5] || 0).toFixed(1);
        } catch (e) {
            // Silent fail or log if needed
        }
        }

        // Start polling every 200ms for smooth updates
...
class RobotController:
    ...
    def print_current_joint_positions_deg_and_force_in_Fz(self):

        # Call the robot API function
        errorcode, joint_pos = self.robot.GetActualJointPosDegree(flag=1)
        ...
        if errorcode == 0:
            ...
            print(f"Error retrieving joint positions. Error code: {errorcode}")
            self.print_current_joint_positions_deg()
...
@app.route('/api/live')
def get_live_data():
    """Return current Fz and joint positions for the frontend"""
    fz = robot_controller.current_forces[2] if len(robot_controller.current_forces) > 2 else 0.0
    # Get joint positions if robot is connected
    joints = [0.0] * 6
    if robot_controller.robot:
        try:
            errorcode, joint_pos = robot_controller.robot.GetActualJointPosDegree(flag=1)
            if errorcode == 0:
                joints = joint_pos
        except:
            pass

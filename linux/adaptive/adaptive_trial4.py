...
# my modification on maytrei mam's new recomendation    
...
previous_trial_peak_force = 0.0  # Store peak force from previous trial for assistance calculation
startup_delay = 2.0
...
def control_loop():
    ...
    global previous_trial_peak_force
    ...
    while True:
        ...
        # Apply assistance based on previous trial's peak force, not current FT reading
        assist_force_applied = 0.0
        current_trial_assist_percent = next_trial_assist_percent
        if current_trial_assist_percent > 0 and previous_trial_peak_force > 1.0:
            assist_force_applied = current_trial_assist_percent * previous_trial_peak_force
        ...
        if err == 0:
            ...
            if state == "bottom" and j3_pos <= TOP_POS + REP_TOL:
                ...
                if f_baseline is None and trial_count <= MAX_BASELINE_TRIALS:
                    ...
                    previous_trial_peak_force = current_peak_abs  # Store for next trial assistance

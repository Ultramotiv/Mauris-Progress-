...
# maitreyi mam recomendation post archana's modifications 
import sys
...
def control_loop():
    ...
    while True:
        ...
        if current_trial_assist_percent > 0 and abs(raw_fz) > 1.0:
            assist_force_applied = current_trial_assist_percent * abs(raw_fz)
            if raw_fz < 0:
            ...
            if state == "bottom" and j3_pos <= TOP_POS + REP_TOL:
                ...
                if f_baseline is None and trial_count <= MAX_BASELINE_TRIALS:
                    ...
                    if trial_count == MAX_BASELINE_TRIALS:
                        ...
                        gui_data["baseline_text"] = f"✅ Baseline Avg: {f_baseline:.2f} N | Threshold: {f_baseline * 0.60:.2f} N"
                        gui_data["assistance_text"] = "Mode: Monitoring (Waiting for data...)"
                        next_trial_assist_percent = 0.0
                        print(f"✅ Baseline: {f_baseline:.2f} N | Trigger: {f_baseline * 0.60:.2f} N")
                    ...
                    if ratio >= 0.70:
                        ...
                        next_trial_assist_percent = 0.50
                        assist_label = "50% Assist"
                        icon = "🔵"
                    elif ratio >= 0.20:
                        next_trial_assist_percent = 0.60
                        assist_label = "60% Assist"
                        icon = "🔵"
                    else:
                        next_trial_assist_percent = 0.60
                        assist_label = "60% Assist (Min Effort)"
                        icon = "🔴"

                    trial_display = f"Trial {trial_count}: {current_peak_abs:.2f} N ({ratio*100:.0f}% of baseline)"
                    if ratio < 0.70:
                        trial_display += f" → {assist_label} next"
                    
                    gui_data["all_trials_display"].append(trial_display)

                    if next_trial_assist_percent > 0:
                        gui_data["assistance_text"] = f"Mode: {icon} {assist_label} in NEXT Trial"
                    else:
                        gui_data["assistance_text"] = "Mode: Monitoring (No assistance)"
                    
                    print(f"🔄 Trial {trial_count}: Peak = {current_peak_abs:.2f} N ({ratio*100:.1f}%) → {assist_label} next trial")

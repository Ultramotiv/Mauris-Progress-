...
def load_user_trajectory(traj_filename, n_joints=6):
    if not os.path.exists(traj_filename):
        raise FileNotFoundError(f"Trajectory file not found: {traj_filename}")

    ext = os.path.splitext(traj_filename)[1].lower()
    ...
    if ext == '.csv':
        ...
        elif df.shape[1] >= n_joints:
            print("CSV does not have joint1..joint6 headers. Using first 6 columns as trajectory.")
            traj = df.iloc[:, :n_joints].values.astype(float)
        else:
            raise ValueError("CSV must contain joint1..joint6 columns (or at least 6 numeric columns).")
        ...
        if isinstance(data, dict):
            if 'trajectory' in data:
                ...
                raise ValueError("JSON must contain key 'trajectory' or 'points'.")
        ...
        for p in points:
            if isinstance(p, dict):
                parsed.append([float(p[f'joint{i+1}']) for i in range(n_joints)])
            else:
                if len(p) < n_joints:
                    raise ValueError("Each trajectory point must have 6 joint values.")
                parsed.append([float(v) for v in p[:n_joints]])
        traj = np.array(parsed, dtype=float)

    elif ext == '.npy':
        traj = np.array(np.load(traj_filename), dtype=float)
        if traj.ndim != 2:
            raise ValueError("NPY trajectory must be a 2D array (N x 6).")
        if traj.shape[1] > n_joints:
            traj = traj[:, :n_joints]

    else:
        raise ValueError("Unsupported trajectory format. Use .csv, .json, or .npy")
...
def execute_remaining_trajectory(dt_local, start_pose, trajectory):
    dists = np.sum((trajectory - np.array(start_pose))**2, axis=1)
    current_idx = np.argmin(dists)
    print(f"\nAUTO FOLLOW from index {current_idx} / {len(trajectory)-1}")

    for k in range(current_idx, len(trajectory)):
        target = trajectory[k]
        loop_start = time.time()
        robot.ServoJ(list(target), [0]*6, int(dt_local*1000))
        elapsed = time.time() - loop_start
        if elapsed < dt_local:
            time.sleep(dt_local - elapsed)

    final_pos = trajectory[-1].tolist()
    print("\n" + "="*50)
    print("TRAJECTORY FINISHED → Goal Reached!")
...
def get_fk_xyz(joint_pos):
    """Return FK XYZ (mm) for a 6-joint position. Returns None on FK failure."""
    try:
...
def build_tcp_trajectory(joint_trajectory):
    """Precompute TCP XYZ trajectory from joint trajectory using FK."""
    tcp_pts = []
...
def _orthonormal_basis_from_tangent(tangent):
    ...
    ref = np.array([0.0, 0.0, 1.0]) if abs(t[2]) < 0.9 else np.array([0.0, 1.0, 0.0])
    u = np.cross(t, ref)
    un = np.linalg.norm(u)
    if un < 1e-9:
...
def visualize_trajectory_with_tunnel(tcp_trajectory, tunnel_radius_mm):
    """Create a basic matplotlib 3D window with trajectory and tunnel rings."""
    ...
    ax.plot(xs, ys, zs, color='tab:blue', linewidth=2.5, label='TCP Trajectory')
    ax.scatter([xs[0]], [ys[0]], [zs[0]], color='green', s=45, label='Start')
    ax.scatter([xs[-1]], [ys[-1]], [zs[-1]], color='red', s=45, label='Goal')
    current_tcp_marker, = ax.plot([xs[0]], [ys[0]], [zs[0]], marker='o', linestyle='',
                                  color='magenta', markersize=7, label='Current TCP')

    # Draw tunnel as circular rings normal to local tangent
    n_points = len(tcp_trajectory)
    stride = max(1, n_points // 80)  # keep plot light
    ...
    # Equal-ish scale for better shape perception
    xrange = np.ptp(xs) if np.ptp(xs) > 1e-6 else 1.0
    yrange = np.ptp(ys) if np.ptp(ys) > 1e-6 else 1.0
    zrange = np.ptp(zs) if np.ptp(zs) > 1e-6 else 1.0
    ...
    plt.pause(0.001)
    return fig, ax, current_tcp_marker


def update_current_tcp_marker(viz_handles, tcp_xyz):
    """Update magenta current TCP marker in existing matplotlib 3D plot."""
    if viz_handles is None or tcp_xyz is None:
        return
    try:
        fig, _, current_marker = viz_handles
        x, y, z = float(tcp_xyz[0]), float(tcp_xyz[1]), float(tcp_xyz[2])
        current_marker.set_data_3d([x], [y], [z])
...
# Cartesian tunnel (mm) around planned trajectory
TUNNEL_RADIUS_MM = 100.0
TUNNEL_RESIST_ENABLE = True
TUNNEL_K = 0.035             # slowdown gain per mm outside tunnel
TUNNEL_MAX_GAIN = 5.0        # cap for slowdown factor
...
traj_file = input("\nEnter trajectory filename (.csv/.json/.npy): ").strip()
if not traj_file:
    print("No trajectory file provided → exiting.")
    sys.exit(0)

print(f"\n{'=' * 60}")
print("LOADING USER TRAJECTORY")
print(f"{'=' * 60}")
raw_trajectory = load_user_trajectory(traj_file, n_joints=6)

# Auto-direction from current joint position:
# if current is closer to trajectory start -> forward, else reverse.
dist_to_start = float(np.sum((np.array(current_pose) - raw_trajectory[0]) ** 2))
dist_to_end = float(np.sum((np.array(current_pose) - raw_trajectory[-1]) ** 2))

if dist_to_start <= dist_to_end:
    direction_mode = "FORWARD"
    planned_trajectory = raw_trajectory.copy()
else:
    direction_mode = "REVERSE"
    planned_trajectory = raw_trajectory[::-1].copy()

# IMPORTANT: Tunnel TCP points are built from exact selected trajectory points only.
planned_tcp_trajectory = build_tcp_trajectory(planned_trajectory)

# Visualize trajectory and tunnel window in a basic matplotlib window
viz_handles = None
try:
    viz_handles = visualize_trajectory_with_tunnel(planned_tcp_trajectory, TUNNEL_RADIUS_MM)
except Exception as e:
    print(f"Visualization error: {e}")

print(f"Loaded trajectory from: {traj_file}")
print(f"Points: {len(planned_trajectory)}")
print(f"Direction selected from current pose: {direction_mode}")
print(f"Start (selected): {planned_trajectory[0]}")
print(f"Goal  (selected): {planned_trajectory[-1]}")
print(f"Current pose distance² -> start: {dist_to_start:.2f}, end: {dist_to_end:.2f}")
print(f"Tunnel radius: {TUNNEL_RADIUS_MM:.1f} mm around trajectory")
print(f"{'=' * 60}\n")
...
def hybrid_control_loop():
    ...
    tcp_trajectory = planned_tcp_trajectory
    n_steps = len(trajectory)
    start_ref = trajectory[0]
    goal = trajectory[-1]

    free_indices = list(range(6))

    print("HYBRID CONTROL STARTED. Push with force to move along selected trajectory direction.")
    print("Trajectory will complete ONLY by user force guidance (no auto-complete).")
    print(f"A {TUNNEL_RADIUS_MM:.0f} mm tunnel is allowed around trajectory.")
    print("No spring-back correction; outside tunnel only motion resistance is applied.\n")
    ...
    while True:
        ...
        tcp_now_mm = None
        tcp_path_dist_mm = 0.0
        tunnel_excess_mm = 0.0
        tcp_err, tcp_pose = robot.GetActualTCPPose(flag=1)
        if tcp_err == 0 and tcp_pose is not None and len(tcp_pose) >= 3:
            tcp_now_mm = np.array([float(tcp_pose[0]), float(tcp_pose[1]), float(tcp_pose[2])], dtype=float)
            tcp_dists = np.sum((tcp_trajectory - tcp_now_mm) ** 2, axis=1)
            tcp_path_dist_mm = float(np.sqrt(np.min(tcp_dists)))
            tunnel_excess_mm = max(0.0, tcp_path_dist_mm - TUNNEL_RADIUS_MM)
            update_current_tcp_marker(viz_handles, tcp_now_mm)
        ...
        # Tunnel resistance only: no spring-back, just slower motion outside tunnel
        if TUNNEL_RESIST_ENABLE and tunnel_excess_mm > 0.0:
            gain_boost = min(TUNNEL_MAX_GAIN, TUNNEL_K * tunnel_excess_mm)
            speed_scale = 1.0 / (1.0 + gain_boost)
            projected_delta = projected_delta * speed_scale
            proj_length = proj_length * speed_scale
        ...
        if not rep_active and cur_speed > START_THRESHOLD:
            ...
            if err == 0:
                start_dists = np.sum((start_ref - cur) ** 2)
        ...
        # ----------------------------------------------------------
        # 9. PAUSE HANDLING (NO AUTO TAKE-OVER)
        # ----------------------------------------------------------
        ...
        if rep_active:
            if pause_condition:
                if stop_timer is None:
                    ...
                    print("\nUser paused ≥ 0.5 s → Holding position. Apply force to continue.")
                    velocity = [0.0] * 6
        ...
        # ----------------------------------------------------------
        # 10. GOAL REACHED → STAY IN MANUAL MODE (NO EXIT)
        # ----------------------------------------------------------
        ...
        if goal_dist < GOAL_TOLERANCE:
            ...
            print("Manual mode kept ON. Reversing direction on same trajectory...\n")

            # Reverse direction so user can push again on same path
            trajectory = trajectory[::-1].copy()
            tcp_trajectory = tcp_trajectory[::-1].copy()
            start_ref = trajectory[0]
            goal = trajectory[-1]
        ...
        # ----------------------------------------------------------
        status_msg = (
            f"Rep:{rep_active} | Spd:{cur_speed:5.2f}°/s | Idx:{current_idx:3d}/{n_steps-1} | "
            ...
            f"Fx:{forces[0]:5.2f}N | Fz:{forces[2]:5.2f}N | d_tunnel:{tcp_path_dist_mm:5.1f}mm"
        )

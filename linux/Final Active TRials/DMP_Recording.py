...
def start_recording():
    ...
    """
    global recording_active, recording_data, recording_thread, recording_filename
    
    if recording_active:
        print("Recording is already active!")
        return

    # Filename is expected to be set before 's'. Fallback only if missing.
    ...
    print(f"Recording will be saved to: {recording_filename}")
...
def stop_recording():
    ...
    # Use filename already taken at start ('s')
    filename = recording_filename
    if not filename:
        filename = f"joint_recording_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        print(f"Warning: filename was not set. Using default: {filename}")
...
def main():
    ...
    global recording_active, recording_thread, recording_filename
    ...
    try:
        ...
        print("=" * 60)
        print(f"\nRobot Configuration:")
        print(f"- Impedance control: On when recording starts")
        print(f"- Gravity compensation: On when recording starts")
        ...
        print("  2) Press 's' to start recording OR 'q' to quit")
        print("  3) While recording, press 'g' to stop and save")
        print("=" * 60)
        print("\nRobot is ready.\n")

        # Ask filename FIRST, before asking for start/quit
        ...
        recording_filename = filename
        print(f"Planned output file: {recording_filename}")

        # Explicit first action prompt
        first_cmd = input("Press 's' to start recording or 'q' to quit: ").strip().lower()
        if first_cmd == 'q':
            print("\nExiting program...")
            return
        elif first_cmd == 's':
            ...
            # Ask specifically for 'g' while recording
            while recording_active:
                stop_cmd = input("Press 'g' to stop recording: ").strip().lower()
                if stop_cmd == 'g':
                    stop_recording()
                    if drag_mode_enabled:
                        custom_drag_teach_mode(enable=False)
                        drag_mode_enabled = False
                        print("Drag mode turned OFF after pressing 'g'.")
                else:
                    print("Invalid input. Please press 'g' to stop recording.")
        else:
            print("Invalid first command. Exiting program...")
            return
        ...
        while True:
            ...
            if user_input == 's':
                ...
                if recording_active:
                    ...
                    print("No active recording. Press 's' to start recording.")

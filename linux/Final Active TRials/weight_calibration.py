...
force_to_translation = 60.0   # Scales N  → mm/s²  (higher = more responsive)120
...
# Limb/tool weight target that should remain after baseline calibration
# [Fx, Fy, Fz, Mx, My, Mz]
Limb_weight = [-21.909, 38.790, -550.509, 6.836, -30.253, -1.478]

# If True, avoid hardware FT zero so raw sensor data is preserved.
# Baseline removal is done in software using `baseline_forces` (set to Limb_weight).
software_zero_only = True
...
def init_ft_sensor():
    ...
    if software_zero_only:
        print("FT Sensor initialized (software zero mode: preserving raw FT data).")
    else:
        # First zero — coarse
        robot.FT_SetZero(0)
        time.sleep(0.3)
...
def calibrate_baseline_forces():
    ...
    if software_zero_only:
        ...
        #   calibrated_ft = raw_ft - baseline_forces
        # We choose baseline_forces so that, at rest:
        #   calibrated_ft == Limb_weight
        # Therefore:
        #   baseline_forces = mean_raw_rest - Limb_weight
        ...
        print(f"  Collecting {gravity_compensation_samples} samples for software baseline...")
        force_samples = []
        for _ in range(gravity_compensation_samples):
            ft_data = robot.FT_GetForceTorqueRCS()
            if ft_data[0] == 0:
                force_samples.append([ft_data[1][j] for j in range(6)])
            time.sleep(0.01)

        if not force_samples:
            print("WARNING: Could not capture baseline samples. Using zero baseline.")
            baseline_forces = [0.0] * 6
            return

        mean_raw = [sum(f[i] for f in force_samples) / len(force_samples) for i in range(6)]
        baseline_forces = [mean_raw[i] - Limb_weight[i] for i in range(6)]

        labels = ['Fx', 'Fy', 'Fz', 'Mx', 'My', 'Mz']
        calibrated_rest = [mean_raw[i] - baseline_forces[i] for i in range(6)]
        print(f"  Mean raw rest:   {', '.join(f'{labels[i]}={mean_raw[i]:+.3f}' for i in range(6))}")
        print(f"  Baseline force:  {', '.join(f'{labels[i]}={baseline_forces[i]:+.3f}' for i in range(6))}")
        print(f"  Calibrated rest: {', '.join(f'{labels[i]}={calibrated_rest[i]:+.3f}' for i in range(6))}")
        print(f"  Target limb wt:  {', '.join(f'{labels[i]}={Limb_weight[i]:+.3f}' for i in range(6))}")
...
def control_loop():
    ...
    while not shutdown_requested:
        ...
        # 1) Remove baseline only -> this should retain limb weight at rest
        if baseline_forces is not None:
            calibrated_forces = [raw_forces[i] - baseline_forces[i] for i in range(6)]
        else:
            calibrated_forces = list(raw_forces)

        # 2) Remove known limb weight for interaction/control signal
        comp_forces = [calibrated_forces[i] - Limb_weight[i] for i in range(6)]

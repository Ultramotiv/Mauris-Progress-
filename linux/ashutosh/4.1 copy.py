...
##in this code all joints are free and rest is exactluy like 4.1
    ...
    while True:
        ...
        for force_axis, joint_idx in force_joint_map.items():
            ...
            if joint_idx == 2:
                ...
                # --- CONTROL LOGIC FOR OTHER JOINTS (0, 1, 3, 4, 5) ---
                # Apply simple admittance control for all other joints
                if abs(axis_force) > force_thresholds[joint_idx]:
                    direction = 'positive' if axis_force > 0 else 'negative'
                    print(f"[FORCE] Joint {joint_idx} moved | Force: {axis_force:.2f}N | Direction: {direction} | Joint: {joint_idx}")
                
                # Reverse movement for joint 0 and joint 1 (from imp_trial2.py logic)
                if joint_idx in [0, 1]:
                    axis_force = -axis_force
                
                # Simple admittance control for other joints
                acc = (axis_force - B[joint_idx]*velocity[joint_idx]) / M[joint_idx]
        ...
        # --- FREE ALL JOINTS ---
        # Ensure all joints (0-5) are always free to move, never fixed
        robot.ServoJ(joint_pos=desired_pos, axisPos=[0]*6)
        time.sleep(dt)

#complete code based on only speed so that passive mode is activated based on speed 
#speed < 0.5 the robot moves in passive mode with better performance at the end of passive movement as well 
# good with speed based!!!
#import Robot
import sys
...
HOLD_FORCE_THRESHOLD = 10.0  # N, force needed to exit hold
...
def control_loop():
    ...
    hold_position = False  # <-- NEW: hold joint after passive
    ...
    while True:
        ...
        # --- PASSIVE MODE LOGIC ---
        if current_speed < 0.5 and desired_pos[j] <= PASSIVE_START_LIMIT and not hold_position:
            if not passive_waiting:
                ...
                if elapsed >= 0.5 and passive_direction:
                    ...
                    # --- Hold joint at position until external force ---
                    velocity[j] = 0.0
        ...
        # --- HOLD POSITION LOGIC ---
        if hold_position:
            # Wait until applied force > threshold
            if abs(fz_force) > HOLD_FORCE_THRESHOLD:
                hold_position = False
            else:
                velocity[j] = 0.0

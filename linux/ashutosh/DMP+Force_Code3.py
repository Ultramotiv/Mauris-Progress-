...
class MultiJointDMP:
    def __init__(self, n_joints=6, n_basis=50, max_vel=15.0, max_accel=1000.0):
        ...
        self.trained = False
        self.goal_offset = np.zeros(n_joints)      # ← Relative motion learned
    ...
    def learn_from_csv(self, csv_filename):
        ...
        for j in range(self.n_joints):
            ...
            self.goal_offset[j] = original_goal - original_start   # ← LEARN RELATIVE MOTION
    ...
    # ← GENERATE FULL TRAJECTORY FROM CURRENT POSE + OFFSET
    def generate_trajectory(self, start_pos, dt):
        ...
        goals = y + self.goal_offset          # ← NEW GOAL

        # Update DMP start/goal
...
# ----------------------------------------------------------------------
#  LOCK CONFIGURATION – ALL JOINTS FREE
# ----------------------------------------------------------------------
LOCKED_JOINTS = []          # ← EMPTY → NO JOINT IS LOCKED
...
def hybrid_control_loop():
    ...
    while True:
        ...
        # ----------------------------------------------------------
        # You can customize force-to-joint mapping here
        force_map = [
            (0, forces[0]),   # J1 ← Fx
            (1, forces[2]),   # J2 ← Fz
            (2, -forces[0]),  # J3 ← -Fx
        ...
        # ----------------------------------------------------------
        # 10. GOAL REACHED → hold forever
        # ----------------------------------------------------------
        goal_dist = np.sum((np.array(desired_pos) - goal) ** 2)
        if goal_dist < GOAL_TOLERANCE:
            print(f"\nNew goal reached! (dist²={goal_dist:.2f}) Stopping motion.")
            velocity = [0.0] * 6
            rep_active = False
            while True:
                robot.ServoJ(list(desired_pos), [0] * 6, int(dt * 1000))

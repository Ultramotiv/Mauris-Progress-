#this code is all in one + impedence control 
...
# --- FORCE TO JOINT MAPPING (from imp_trial2.py) ---
force_joint_map = {
    0: 1,  # Fx -> Joint 1
    1: 0,  # Fy -> Joint 0  
    2: 2,  # Fz -> Joint 2
...
ASSISTANCE_LOW = 0.1   # 20% assistance
#my addings 
...
velocity = [0.0]*6
# Initialize initial_joint_pos for deviation calculations
...
# --- MAIN LOOP ---
def control_loop():
    global desired_pos, velocity, home_pos, baseline_forces, rep_count, rep_state, initial_joint_pos
    ...
    while True:
        # Get current joint positions for deviation calculation
        ...
        # --- FORCE-TO-JOINT MAPPING CONTROL ---
        # Process each force axis and map to corresponding joint
        for force_axis, joint_idx in force_joint_map.items():
            axis_force = forces[force_axis]
            
            # Special handling for Joint 2 (main exercise joint)
            if joint_idx == 2:
                j = 2
                fz_force = axis_force
                # Calculate speed for Joint 2
                ...
                # --- PASSIVE MODE LOGIC FOR JOINT 2 ---
                ...
                # --- MODE SWITCHING & ASSISTANCE LOGIC FOR JOINT 2 ---
                ...
                # --- CONTROL LOGIC FOR OTHER JOINTS (0, 1, 3, 4, 5) WITH FAULT TOLERANCE ---
                # Calculate deviation from initial trajectory position
                deviation = current_joint_pos[joint_idx] - initial_joint_pos[joint_idx]
                abs_deviation = abs(deviation)
                # Reverse movement for joint 0 and joint 1 (from imp_trial2.py logic)
                if joint_idx in [0, 1]:
                    axis_force = -axis_force
                if abs_deviation <= FAULT_TOLERANCE:
                    # Within tolerance: apply half of the force (resistance)
                    resist_force = axis_force * 0.5
                    total_force = resist_force
                    if abs(axis_force) > force_thresholds[joint_idx]:
                        direction = 'positive' if axis_force > 0 else 'negative'
                        print(f"[RESIST] Joint {joint_idx} | Force: {axis_force:.2f}N | Resistance: {resist_force:.2f}N | Deviation: {deviation:.2f}° | Direction: {direction}")
                else:
                    # Beyond tolerance: apply strong spring-back force
                    excess_deviation = abs_deviation - FAULT_TOLERANCE
                    # Calculate stiffness based on deviation (gradual increase like imp_trial2.py)
                    spring_stiffness = K_ASSIST + min(excess_deviation / FAULT_TOLERANCE, 1.0) * K_FIELD_GRADIENT * 2.0
                    # Spring force towards initial position (trajectory)
                    spring_force = -spring_stiffness * deviation / force_to_deg
                    # Combine spring force with reduced applied force
                    total_force = spring_force + axis_force * 0.2
                    direction = 'positive' if total_force > 0 else 'negative'
                    print(f"[SPRING-BACK] Joint {joint_idx} | Spring: {spring_force:.2f}N | Force: {axis_force:.2f}N | Deviation: {deviation:.2f}° | Direction: {direction}")
                
                # Update velocity and desired position
                acc = (total_force - B[joint_idx]*velocity[joint_idx]) / M[joint_idx]
                velocity[joint_idx] += acc*dt
                delta = velocity[joint_idx]*dt*force_to_deg
                desired_pos[joint_idx] += delta
        # Safety checks
        safety_ok = True
        for idx in range(6):
            joint_num = idx + 1

...
# FINAL FIXED VERSION – START FROM CURRENT POSE, SAME SHAPE, SAME GOAL
# -------------------------------------------------------------------------
#   • Learns the *shape* from a CSV demonstration (filtered + safe limits)
#   • At execution time:
#        – reads the *current* joint angles → new y0 for every DMP
#        – keeps the *original* CSV goal → g stays the same
#        – replays the **same** velocity/acceleration profile
#   • No overshoot, stops exactly at the CSV goal
# -------------------------------------------------------------------------
#####Perfect code#### change the start but reaches to the same end goal position!!!!
...
class DMP1D:
    ...
    def learn_from_trajectory(self, positions, velocities, accelerations, dt):
        ...
        self.g = positions[-1]                # demo goal  (kept forever)
...
class MultiJointDMP:
    ...
    def learn_from_csv(self, csv_filename):
        ...
        for j in range(self.n_joints):
            ...
            # KEEP ORIGINAL CSV GOAL – will be used at execution
            self.dmps[j].y0 = data['original_start']   # demo start (will be overwritten)
    ...
    # --------------------------------------------------------------------- #
    #  EXECUTION – START FROM *CURRENT* POSE, KEEP ORIGINAL GOAL
    # --------------------------------------------------------------------- #
...
def main():
    print("\n" + "="*60)
    print("DMP REPLAY – START FROM CURRENT POSE – SAME SHAPE")
    print("="*60)
    print("Max Velocity: 15.0°/s | Max Acceleration: 1000.0°/s²")
    print("Stops exactly at the *original* CSV goal")
    print("="*60)

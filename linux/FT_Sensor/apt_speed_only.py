# DONE
# speed only without gravity
#so patient move the arm up and stops applying force the robot comes down but the movement is smooth
...
def control_loop():
    ...
    while True:
        ...
        if mode == "sitting":
            ...
            # Get current joint speed (absolute value)
            current_speed = abs(velocity[j] * force_to_deg)  # deg/s
            
            # Start with raw force (no assistance by default)
            assisted_force = fz_force
            
            # Apply assistance ONLY if force is non-zero AND speed is below target
            if abs(fz_force) > 0.0 and current_speed < TARGET_SPEED:
                # Calculate how much speed boost is needed
                speed_deficit = TARGET_SPEED - current_speed
                
                # Calculate assistance factor (more deficit = more assistance)
                assistance_factor = speed_deficit / TARGET_SPEED
                
                # Apply assistance proportional to the speed deficit
                assistance_force = fz_force * assistance_factor
                assisted_force = fz_force + assistance_force
                
                print(f"★ Speed Assistance Active:\n" 
                      f" Current Speed={current_speed:.2f}°/s\n"
                      f" Target Speed={TARGET_SPEED:.2f}°/s\n"
                      f" Speed Deficit={speed_deficit:.2f}°/s\n"
                      f" Raw Force={fz_force:.2f}N\n"
                      f" Assistance={assistance_force:.2f}N\n"
                      f" Total Force={assisted_force:.2f}N")
                      
            elif abs(fz_force) > 0.0 and current_speed >= TARGET_SPEED:
                print(f"✓ Target Speed Achieved - No Assistance: {current_speed:.2f}°/s >= {TARGET_SPEED:.2f}°/s")
            elif abs(fz_force) == 0.0:
                print(f"No Force Applied - No Assistance: Force={fz_force:.2f}N")

            # Apply physics-based control with assisted force
            if abs(assisted_force) < 1.0 and current_speed < 0.5:
                # Very low force and speed - use spring to home position
                home_pos[j] = desired_pos[j]
                spring_force = -K[j]*(desired_pos[j]-home_pos[j])/force_to_deg
                acc = (spring_force - B[j]*velocity[j]) / M[j]
            else:
                # Use assisted force for motion control
            ...
            # Debug print
            print(f"[STANDING] Speed={current_speed:.2f}°/s | Target={TARGET_SPEED:.2f}°/s | "
                  f"Force={fz_force:.2f}N | Assisted={assisted_force:.2f}N | Vel={velocity[j]:.4f}")

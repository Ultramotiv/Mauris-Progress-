...
# Joint 2 limits
JOINT2_MIN_LIMIT = 60.0    # degrees 
JOINT2_MAX_LIMIT = 122.0   # degrees
...
ASSISTANCE_ANGLE = 100.0   # degrees - robot will auto-continue from this point
...
def control_loop():
    ...
    while True:
        ...
        safety_ok = True
        for j in range(1, 4):  # Joints 1,2,3 (indices 1,2,3)
            if not is_within_safety_limits(j, desired_pos[j]):
                print(f"Invalid joint angle for Joint {j}: {desired_pos[j]:.2f}° (Safety range: {JOINT_SAFETY_LIMITS[j][0]}° to {JOINT_SAFETY_LIMITS[j][1]}°)")
                safety_ok = False
        if not safety_ok:
            time.sleep(dt)
            continue

...
def control_loop():
    ...
    while True:
        ...
        if (0.0 < abs(fz_force) < 4.0  and desired_pos[j] <= PASSIVE_START_LIMIT): #and 0.0 < current_speed < AVG_SPEED
            if not passive_waiting:
                ...
                if not (0.0 < abs(fz_force) < 4.0 and desired_pos[j] <= PASSIVE_START_LIMIT): #0.0 < current_speed < AVG_SPEED and
                    passive_waiting = False
                    passive_timer = None
                    passive_direction = None
                    continue
                ...
                if elapsed >= 1.0:
                    ...
                    if passive_direction == "up":
                        ...
                        while abs(desired_pos[j] - JOINT2_MAX_LIMIT) > 0.5:
                            assist_force = 10.0

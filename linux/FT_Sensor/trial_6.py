...
M = [5.0, 5.0, 5.0, 5.0, 5.0, 5.0]  # Reduced mass for joints 3,4,5 for easier movement
B = [10.0, 10.0, 10.0, 10.0, 10.0, 10.0]  # 50% damping reduction for joints 3,4,5 (was 20.0, now 10.0)
...
force_to_deg = 5  # 1N --> 10degree move for robot
...
combination_threshold = 1.0  # Both forces must exceed this to activate combination
...
print("Admittance control started - ALL JOINTS ACTIVE:")
print("Joint 0: Force Y (>0.3N) | Joint 1: Force X (>0.3N) | Joint 2: Force Z (>0.3N)")
print("Joint 3: Force X + Z (>0.3N each) | Joint 4: Force Y + Z (>0.3N each) | Joint 5: Force Y + X (>0.3N each)")
...
def control_loop():
    ...
    while True:
        ...
        # Process simple mappings for joints 0, 1, 2 (NOW ACTIVE)
